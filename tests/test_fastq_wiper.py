...
def fw_run(fw):
	...
	alphabet: str = 'ACGTN'
	arg = argparse.Namespace(fastq_in = fastq_in, fastq_out = fastq_out, log_out = log_out, log_frequency = log_frequency, alphabet = alphabet)
	fw.run(arg)
...
def test_create_fastq_write_file_handler(fw):
		assert isinstance(fw.create_fastq_write_file_handler('./tests/testdata/new.fastq'), TextIOWrapper)
...
def test_check_wellformed_qual_line(fw, fw_bad_fastq):
	...
	assert fw.check_qual_line(fw.read_next_line(fw_bad_fastq, 100)) == '#A<AA/EEEEEEEEEEEEEAEEEEEEEEE#EEEE#EE#EEE#EE<###E#EEEE/EEEEEEE/EEEEEEAEEEAEE'
