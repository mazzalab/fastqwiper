...
class GatherReport(WiperTool):
    ...
    def set_parser(self, parser: argparse.ArgumentParser):
        parser.add_argument("-r", '--reports', nargs='+',
                            help='List of report files', required=True)
        parser.add_argument("-f", '--final_report',
                            help='The final report file', required=True)
        # Add a version flag that prints the version and exits
        parser.add_argument('-v', '--version', action='version',
                            version=self.version(), help='It prints the version and exists')
    ...
    def run(self, argv: argparse.Namespace):
        ...
        with open(final_report_filepath, 'w') as file_out:
            ...
            file_out.write(f"{WELLFORMED}: {final_report[WELLFORMED]} ({round(
                (final_report[WELLFORMED] / final_report[TOTAL_LINES]) * 100, 2)}%)" + "\n")
            file_out.write(f"{CLEAN}: {final_report[CLEAN]}" + "\n")
            file_out.write(f"{MISPLACED_HEADER}: {final_report[MISPLACED_HEADER]} ({round(
                (final_report[MISPLACED_HEADER] / final_report[TOTAL_LINES]) * 100, 2)}%) of which {final_report[RECOVERED_HEADER]} fixed" + "\n")
            file_out.write(f"{BAD_SEQ}: {final_report[BAD_SEQ]} ({round(
                (final_report[BAD_SEQ] / final_report[TOTAL_LINES]) * 100, 2)}%)" + "\n")
            file_out.write(f"{BAD_PLUS}: {final_report[BAD_PLUS]} ({round(
                (final_report[BAD_PLUS] / final_report[TOTAL_LINES]) * 100, 2)}%)" + "\n")
            file_out.write(f"{BAD_QUAL}: {final_report[BAD_QUAL]} ({round(
                (final_report[BAD_QUAL] / final_report[TOTAL_LINES]) * 100, 2)}%)" + "\n")
            file_out.write(f"{LENGTH_SEQ_QUAL}: {
                           final_report[LENGTH_SEQ_QUAL]}" + "\n")
            file_out.write(f"{BLANKS}: {final_report[BLANKS]} ({round(
                (final_report[BLANKS] / final_report[TOTAL_LINES]) * 100, 2)}%)" + "\n")
            ...
            for line in file:
                ...
                if line.startswith(TOTAL_LINES):
                    ...
                    if (m):
                        ...
                        raise ValueError(
                            f"TOT_LINES match failed for line: {right}")
                    ...
                    if m:
                        ...
                        raise ValueError(
                            f"WELLFORMED match failed for line: {right}")
                    ...
                    if m:
                        ...
                        raise ValueError(
                            f"CLEAN match failed for line: {right}")
                    ...
                    if m:
                        ...
                        raise ValueError(
                            f"BAD_SEQ match failed for line: {right}")
                    ...
                    if m:
                        ...
                        raise ValueError(
                            f"BAD_PLUS match failed for line: {right}")
                    ...
                    if m:
                        ...
                        raise ValueError(
                            f"BAD_QUAL match failed for line: {right}")
                    ...
                    if m:
                        ...
                        raise ValueError(
                            f"BLANKS match failed for line: {right}")
